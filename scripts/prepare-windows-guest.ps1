...
try {
  ...
  Set-ItemProperty -Path $activeSetupKey -Name Version -Value "1,0,0,5"
  ...
  Set-ItemProperty -Path "HKLM:\Software\Microsoft\Windows\CurrentVersion\Run" -Name NebulaVMWallpaper -Value 'powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File C:\NebulaVM\apply-wallpaper.ps1'
...
"@
  ...
  if (Test-Path -LiteralPath $offlineDefaultUser -PathType Leaf) {
    ...
    try {
      ...
      & reg.exe delete "HKLM\NebulaDefaultUser\Control Panel\Desktop" `
        /v TranscodedImageCache /f 2>$null | Out-Null
      & reg.exe delete "HKLM\NebulaDefaultUser\Control Panel\Desktop" `
        /v TranscodedImageCache_000 /f 2>$null | Out-Null
      & reg.exe add "HKLM\NebulaDefaultUser\Software\Microsoft\Windows\CurrentVersion\Explorer\Wallpapers" `
      ...
      & reg.exe delete "HKLM\NebulaDefaultUser\Software\Microsoft\Windows\CurrentVersion\Explorer\Wallpapers" `
        /v TranscodedImageCache /f 2>$null | Out-Null
    } finally {
