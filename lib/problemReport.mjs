...
const FALLBACK_PROFANITY_WORDS = Object.freeze([
  "ass",
  "asshole",
  "bastard",
  "bitch",
...
const loadProfanityWords = () => {
  try {
    const source = readFileSync(new URL("../data/profane-words.json", import.meta.url), "utf8");
    const parsed = JSON.parse(source);
    if (!Array.isArray(parsed)) {
      throw new Error("Profanity word list must be a JSON array.");
    }
    return parsed;
  } catch (error) {
    console.warn(
      "NebulaVM report moderation is using its fallback profanity list.",
      error?.message || error,
    );
    return FALLBACK_PROFANITY_WORDS;
  }
};

const normalizeProfanityWords = (words) => {
...
const buildProfanityPattern = (words, flags) => {
  const source = words
    .map((word) => word.split(/\s+/).map(escapeRegExp).join("[\\s._-]+"))
    .join("|");
  return new RegExp(`(?<![a-z0-9])(?:${source})(?![a-z0-9])`, flags);
};

export const PROFANITY_WORDS = Object.freeze(
  normalizeProfanityWords([...FALLBACK_PROFANITY_WORDS, ...loadProfanityWords()]),
);

export const STRONG_PROFANITY_WORDS = PROFANITY_WORDS;

export const PROFANITY_STRIKE_WINDOW_MS = 24 * 60 * 60 * 1000;

const cleanText = (value) => String(value || "").replace(/\r\n/g, "\n").trim();
const emailPattern = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
const profanityPattern = buildProfanityPattern(PROFANITY_WORDS, "i");
const strongProfanityPattern = buildProfanityPattern(STRONG_PROFANITY_WORDS, "i");
const strongProfanityRedactionPattern = buildProfanityPattern(
