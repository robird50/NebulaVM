...
The hosted Netlify site is a static launcher. It does not publish, discover, or
store EMUSTAR host tokens. Remote EMUSTAR sessions require the private browser
link generated by the Windows host.

NebulaVM does not include ads, analytics, third-party trackers, or external
cloud file picker integrations. Browser-selected ISO files are sent only to the
selected EMUSTAR host when a user explicitly stages media for that host.
