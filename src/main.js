...
const state = {
  ...
  hostAutopilotRecoveryPromise: null,
    ...
    <section class="display-choice-panel faq-panel popup-motion-panel" id="faqPanel" tabindex="-1">
      ...
      <div class="faq-sections">
        <section class="faq-section">
          <h3>General</h3>
          <details><summary>What is NebulaVM?</summary><p>NebulaVM is an open-source browser interface for virtual machines and emulators. Browser-compatible modes run on the visitor's device; Hyper-V, Remote VM, and other native modes connect to NebulaVM's configured Windows host.</p></details>
          ...
          <details><summary>Do I need to install anything?</summary><p>No installation is required for a visitor using the website. The computer providing host-backed features must have NebulaVM Host and the required virtualization software installed, but ordinary visitors do not configure that computer.</p></details>
          ...
          <details><summary>Which operating systems can I run?</summary><p>Support depends on the runtime and CPU architecture. NebulaVM can run compatible legacy x86 systems and lightweight Linux images in the browser, plus larger Linux and Windows guests through host-backed runtimes. An image is not guaranteed to boot merely because it is an ISO.</p></details>
        </section>
        ...
        <section class="faq-section">
          ...
          <details><summary>Does it work on mobile devices?</summary><p>Yes, but the public mobile experience is intentionally lighter. Phones and tablets can use Remote VM links and supported browser emulators. Direct Hyper-V setup, native QEMU, public Android, and AVD Management are restricted because they require more host resources and desktop-oriented controls.</p></details>
        ...
        <section class="faq-section">
          ...
          <details><summary>What is the Windows 11 Template?</summary><p>It is a prepared Windows 11 virtual disk for the public Hyper-V runtime. Selecting it creates or resumes a private child disk for that browser device, avoiding a complete Windows installation every time.</p></details>
          <details><summary>Can two visitors use public Hyper-V at once?</summary><p>Not on the current public host. It has one Hyper-V machine and serves one private owner at a time. A second visitor is asked to wait instead of being shown or allowed to control the first visitor's screen.</p></details>
          <details><summary>What does a Remote VM link do?</summary><p>A Remote VM link securely points to the currently running Hyper-V session; it does not create another VM. The link contains a unique session identifier and stops working when that session ends or is replaced.</p></details>
          ...
          <details><summary>Can I save my virtual machine?</summary><p>Some runtimes support saved state or persistent disks. The Windows 11 Template can resume a private disk for the same browser device, but public-host disks are temporary and may be removed automatically to protect storage. Do not use NebulaVM as your only copy of important data.</p></details>
          <details><summary>What does Request new disk do?</summary><p>It permanently removes that browser device's current private Hyper-V child disk and prepares a clean replacement from the template. Files and changes on the previous private disk cannot be recovered afterward.</p></details>
        ...
        <section class="faq-section">
          ...
          <details><summary>Are my files private?</summary><p>Stored images and private Hyper-V disks are tied to a browser device identity. The host also blocks other visitors from requesting the active screen, controls, VNC credentials, or session ID. Even with those protections, do not upload highly sensitive data to a public host.</p></details>
          ...
          <details><summary>Are private Windows disks stored forever?</summary><p>No. They are space-saving differencing disks, but they grow as Windows writes data. Inactive private disks can be removed after a retention period or when the host reaches its storage limit.</p></details>
          <details><summary>What happens if my private disk was deleted?</summary><p>The next Windows 11 Template launch creates a new private differencing disk from the read-only base template. The deleted disk's personal files and settings do not return.</p></details>
        </section>

        <section class="faq-section">
          <h3>Performance</h3>
          <details><summary>Why is my VM running slowly?</summary><p>Browser-local speed depends mostly on the visitor's device. Host-backed speed depends on the public host's CPU, available RAM, disk activity, and the network between the host and visitor. A higher frame-rate target cannot remove network latency or create hardware resources that are unavailable.</p></details>
          <details><summary>How much RAM should I allocate?</summary><p>Use the smallest amount the guest can run comfortably with. The public Windows host has limited memory, so 1-2 GB is the practical range for lighter guests. Windows 11 may use dynamic memory and can still slow down when the host is busy.</p></details>
        ...
        <section class="faq-section">
          ...
          <details><summary>My VM is stuck on a black screen. What should I do?</summary><p>Check the Mission log and wait briefly if the guest has just resumed. A black screen can mean Windows is loading, the console is changing from the setup mirror to VNC, or the display connection failed. If there is no progress, use End session once and relaunch; repeatedly pressing Start can make recovery slower.</p></details>
          ...
          <details><summary>Why can't I connect to the public host?</summary><p>The NebulaVM host or its secure browser tunnel may be restarting, offline, blocked by a network, or temporarily busy. This is normally a NebulaVM service problem, not something a visitor must repair on their own computer.</p></details>
        ...
        <section class="faq-section">
          ...
          <details><summary>Can the administrator monitor my VM?</summary><p>Only during disclosed safety monitoring. If an administrator opens the safety view, the VM viewport receives a green border and an "Admin is monitoring for safety purposes" pill. Browser frame uploads stop when monitoring ends. Never enter highly sensitive information into a publicly hosted VM.</p></details>
        </section>
...
const recoverHostedHyperVConnection = async () => {
  if (!isNetlifyLauncher || state.hyperVAutopilotRecoveryInProgress) return false;
  if (state.hostAutopilotRecoveryPromise) return state.hostAutopilotRecoveryPromise;
      ...
      while (Date.now() < deadline) {
        ...
        if (host?.publicUrl) {
          try {
            ...
            if (registryCheck === 1 || registryCheck % 5 === 0) {
              appendAutopilotTerminal(
                `[bridge] check ${registryCheck}: the registered public bridge is not responding.`,
                { kind: "check" },
              );
            }
          }
  ...
  state.hostAutopilotRecoveryPromise = recoveryTask;
  try {
    return await recoveryTask;
  } finally {
    state.hostAutopilotRecoveryInProgress = false;
    if (state.hostAutopilotRecoveryPromise === recoveryTask) {
      state.hostAutopilotRecoveryPromise = null;
    }
  }
};
...
const selectWindows11Template = async ({ boot = false, allowRecovery = true } = {}) => {
  ...
  try {
    ...
    clearWindowsTemplateSelection();
    if (isHostedHyperVOfflineError(error) && allowRecovery) {
      log("Windows 11 Template is waiting for Autopilot to restore the public host.");
      const recovered = await recoverHostedHyperVConnection();
      if (recovered) {
        log("Autopilot restored the host. Retrying Windows 11 Template automatically.");
        await selectWindows11Template({ boot, allowRecovery: false });
        return;
      }
    }
...
const fetchNetlifyHostRegistry = async () => {
  ...
  try {
    ...
    const data = await response.json();
    if (!data.ok || data.stale || !data.host?.publicUrl || !data.host?.accessToken) return null;
